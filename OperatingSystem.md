...
3. 银行家算法
    ...
    - 当用户进程首次申请资源时,要确定改进程的资源最大需求量,如果系统现存的资源满足它最大需求量则按当前的申请量分配资源;
    - 进程在执行中申请资源时,先验证它所占用的资源和此次申请资源数有没有超过最大需求量(也就是有没有守诚信),然后在验证系统的现存资源数是否能满足该进程尚需的最大资源数.
